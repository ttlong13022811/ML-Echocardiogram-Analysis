...
# Data transforms
training_transforms = tf.Compose([tf.Resize((256, 256)),
                                          tf.RandomCrop((224, 224)),
                                          tf.RandomHorizontalFlip(),
                                          tf.ToTensor(),
                                          tf.Normalize(mean=[0.485, 0.456, 0.406],
                                                    std=[0.229, 0.224, 0.225])])
testing_transforms = tf.Compose([tf.Resize((256, 256)),
...
class ConvNet(pl.LightningModule):
    ...
    def forward(self, x):
        # W - F + 1
        # -> n, 3, 32, 32
        x = self.pool(func.relu(self.conv1(x)))  # -> n, 6, 14, 14
        x = self.pool(func.relu(self.conv2(x)))  # -> n, 16, 5, 5
        x = x.view(-1, 16 * 5 * 5)  # -> n, 400
        x = func.relu(self.fc1(x))  # -> n, 120
        x = func.relu(self.fc2(x))  # -> n, 70
        x = self.fc3(x)  # -> n, 3
        return x
